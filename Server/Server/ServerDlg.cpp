...
MYSQL_ROW row;
MYSQL_RES *m_res;
MYSQL *connection = NULL, *connection2 = NULL, mysql, mysql2, mysql3;
...
CString *uT = new CString[Socket_Status];
CString *uK = new CString[Keyword_Status];
CString *uid = new CString[Keyword_Status];
...
void CServerDlg::OnBnClickedOk()
{
	...
	if (p == NULL)  //쓰레드생성실패시에러메시지
		AfxMessageBox(_T("thread 1 Error"));
	else
		AfxMessageBox(_T("thread Success"));
...
UINT CServerDlg::Y_Thread(LPVOID pParam)
{
	...
	while (true)
	{
		...
		query.Format("select * from notice");
		...
		for (int i = 0; i < 15; i++)
		{
			...
			{
				//query.Format("insert into testidx values(\"%d\",\"%s\",\"%s\")", 0, Title_Phashing.GetAt(q), Title.GetAt(i));
...
UINT CServerDlg::Keyword_Store(LPVOID pParam)
{
	CString query;
	SetCursor(LoadCursor(NULL, IDC_WAIT));
	mysql_init(&mysql2);
