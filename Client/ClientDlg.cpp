...
void CClientDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_KEYWORD, m_List_Ctrl);
}
...
BOOL CClientDlg::OnInitDialog()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		m_pClientSock->Connect("127.0.0.1", 700);	
		...
		for (int i = 0; i < 10; i++)
		{
			...
			m_LIST_RECEIVE.InsertItem(LVIF_TEXT | LVIF_STATE, i, strText,
			...
			m_LIST_RECEIVE.SetItemText(i, j, strText);
			}
		}
		...
		{
			GetDlgItem(IDC_RADIO_STATE)->EnableWindow(FALSE); // 상태 버튼 비활성화
			GetDlgItem(IDC_LIST_KEYWORD)->EnableWindow(FALSE); // 알람 키워드 리스트 비활성화
...
void CClientDlg::OnNMDblclkListReceive(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	int nCount = m_LIST_RECEIVE.GetItemCount();	
	...
	{	
		if (i==nItem)
		{
			::ShellExecute(NULL, "open", "chrome.exe", Link_buffer[(nCount-1)-i], NULL, SW_SHOW);
		}			
	}	
